...
Hungarian<T>::Hungarian(Hungarian &&h) noexcept
    : m_mx_og{std::move(h.m_mx_og)}, m_mx_wrk{std::move(h.m_mx_wrk)},
      ...
      m_is_solved{std::move(h.m_is_solved)}, m_solution{
                                                 std::move(h.m_solution)} {}
...
template <typename T> Hungarian<T>::~Hungarian() {}

template <typename T> void Hungarian<T>::Solve() {
  while (m_next_step != 0) {
    switch (m_next_step) {
    case 1:
      Step1();
...
template <typename T> void Hungarian<T>::PrintLines() const {
...
template <typename T> void Hungarian<T>::PrintPrimes() const {
  const std::string red{"\033[41m"};
  const std::string green{"\033[42m"};
  const std::string blue{"\033[44m"};
  const std::string reset_color{"\033[0m"};
...
template <typename T>
Matrix<typename Hungarian<T>::HungarianNotation>
Hungarian<T>::SimpleMatrixToHungarianMatrix(const Matrix<T> &simple_matrix) {
...
template <typename T> bool Hungarian<T>::MatrixIsSquare() const {
...
template <typename T> void Hungarian<T>::Step1() {
  for (size_t cur_row{0}; cur_row < m_mx_wrk.GetRows(); ++cur_row) {
...
template <typename T> void Hungarian<T>::Step5() {
  ...
  for (size_t cur_zero{0}; cur_zero < zeroes_in_series.size(); ++cur_zero) {
    if (m_mx_wrk(zeroes_in_series[cur_zero].row, zeroes_in_series[cur_zero].col)
            .is_starred == true) {
...
template <typename T>
void Hungarian<T>::FindStarredZeroInCol(
    Location2D &cur_pos, std::vector<Location2D> &zeroes_in_series,
...
template <typename T>
void Hungarian<T>::FindPrimedZeroInRow(
    Location2D &cur_pos, std::vector<Location2D> &zeroes_in_series,
    int &next_step_in_series) {
