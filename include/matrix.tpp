...
template <typename T> size_t Matrix<T>::GetRows() const { return m_rows; }

template <typename T> size_t Matrix<T>::GetCols() const { return m_cols; }
...
template <typename T> Matrix<T> &Matrix<T>::operator=(Matrix<T> &&m) noexcept {
  if (this == &m) {
    return *this;
  }
